...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const adminbotnumber = ["6283815956151@s.whatsapp.net"] // GANTI NOMOR LU
			const frendsowner = ["6283815956151@s.whatsapp.net"] // GAUSAH DIGANTI!!!
			...
			case 'owner':
				...
				case 'fake':          
                    ...
                    break
                case 'fakereplay':
                   client.reply(from, 'ange mas', 'mending lari', "0838-15956151")
                   break
                case 'but':
               ...
               case 'infogempa':
			    ...
			    case 'otagall':
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘 MANCA BOT 〙', members_id, true)
					break
				case 'clearall':
					if (!isOwner) return reply('Kamu siapa?')
				...
				//frendowner
				case `addadmin`:
                   if (!isOwner) return reply('Perintah ini hanya bisa di gunakan oleh Owner Manca!', id)
                   for (let i = 0; i < mentionedJidList.length; i++) {
                   adminNumber.push(mentionedJidList[i])
                   fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                   reply('Success Menambahkan Admin Manca!')
				   }
                   break
				case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
